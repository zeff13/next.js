...
const nextDevWorkflow =
  ...
  async ({ turbopack, page }) => {
    ...
    try {
      ...
      let version
      try {
        version = await pageInstance.evaluate('window.next.version')
      } catch (e) {
        // eslint-disable-next-line no-console
        console.log('Page did not load correctly.')
      }
      // Convert version into a number that can be reported
      const versionNumber = version
        ? `${version}.99.99`
            .split('.')
            .slice(0, 4)
            .map((n) => parseInt(n, 10))
            .reduce((t, n) => t * 100 + n, 0)
        : 0

      await reportMeasurement('next version', versionNumber, 'version')
